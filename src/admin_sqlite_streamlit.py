...
st.write(f"Selected Record: {selected_record['ID']} - **{selected_record['Title']}** - {selected_record['Description']} Status:{selected_record['Duplicate_Status']}")
st.write(f"Duplicate Status: {selected_record['Duplicate_Status']}")

# Button to mark selected record
if st.button("Mark Selected Record"):
    conn = sqlite3.connect('activities.db')
    cursor = conn.cursor()
    
    try:
        st.write(f"Selected Id:{selected_record['ID']}")
        
        #Update duplicate_status for the selected record
        cursor.execute("UPDATE activities SET duplicate_status = 'C' WHERE id = ?", (int(selected_record['ID']),))
        
        #Print the rowcount value
        st.write(f"Number of rows affected: {cursor.rowcount}")
        
        if cursor.rowcount > 0:
            st.success("Record has been marked.")
        else:
            st.warning("No record found with the selected ID.")
